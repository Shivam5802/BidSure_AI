...
export default function TenderComplianceReportWebPage({ params }: ComplianceReportPageProps) {
  ...
  const loadReport = async () => {
    setLoading(true);
    try {
      const report = await reportsApi.generateTenderReport(tenderId);
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {loading ? (
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-12 text-center text-slate-500">
            ...
            <p className="text-xs font-medium">Loading tender compliance report...</p>
          </div>
        ) : activeReport ? (
          <>
